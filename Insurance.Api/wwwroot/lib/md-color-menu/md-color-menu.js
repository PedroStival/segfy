(function() {
  ...
  function mdPickerColors($mdColorPalette, $mdPanel) {
    ...
    angular.forEach($mdColorPalette, function(swatch, swatchName) {
      ...
      angular.forEach(swatch, function(color, colorName) {
        ...
        var colorObject = { name: name, hex: hex, editedName: '', style: {'color': foregroundColor, 'background-color': backgroundColor}};
  ...
  function mdColorMenuDirective() {
    return {
      ...
      template: [
        ...
        '  <md-menu-content class="md-cm">',
          '    <div style="padding:10px"> <div class="form-group form-md-line-input form-md-floating-label" style="margin:0"> <input type="text" ng-model="vm.editedName" name=""colopicker" id="colopicker" class="form-control edited"><label for="colopicker">Escolha o nome da cor</label></div></div>',
        '    <div class="md-cm-swatches" layout="row">',
        '      <div ng-repeat="swatch in vm.colors" layout=column>',
        '        <div ng-repeat="color in swatch" class="md-cm-color" ng-style="color.style" ng-click="vm.selectColor(color,vm.editedName)" layout="row" layout-align="center center">',
        '          <span ng-if="color.name == vm.color.name">&#10004;</span>',
  ...
  function mdColorMenuController(mdPickerColors) {
    ...
    function selectColor(color, editedName) {
        vm.color = color;
        vm.color.editedName = editedName;
      vm.editedName = '';
    }
  }
